...
output: 
  rmarkdown::html_vignette:
    toc: true
vignette: >
  %\VignetteIndexEntry{Advanced Uses}
...
These live search examples are shown without execution. They require internet access when run interactively; the NPPES database is refreshed daily, so registry results may change over time.
...
The following executable example uses the bundled `npi::npis` dataset, not the
results of the live searches above. It demonstrates combining two saved batches
and then summarizing and flattening the combined records without internet access.
