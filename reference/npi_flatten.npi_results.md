...
because there are no columns to unnest. All input keys are retained even
when their selected nested data is empty, regardless of the order of
`cols`. If all selected nested data is empty, only the keys are
returned. When multiple nested tables have more than one row for a key,
their rows are combined in all possible combinations.
