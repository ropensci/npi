 ...
 * `flatten_npi()`: Flatten one or more list columns from a search result, joined by NPI number.
...
`search_npi()` exposes nearly all of the NPPES API's [search parameters](https://npiregistry.cms.hhs.gov/registry/help-api). Let's say we wanted to find up to 10 organizational providers with primary locations in New York City:
...
Run `summary()` on your results to see a more human-readable overview of what we've got:
...
Now we're ready to do whatever else we need to do with this data. Under the hood, `flatten_npi()` has done a lot of data wrangling for us:
...
Use `is_valid_npi()` to check whether each element of a vector of candidate numbers is a valid NPI number:
